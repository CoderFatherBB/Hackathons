...
if st.session_state.results:
    ...
    if "error" in st.session_state.results:
        ...
        # Check if this is a deep research result or standard research result
        is_deep_research = "deep_research" in st.session_state.results
        
        if is_deep_research:
            # Create tabs for deep research view
            tab1, tab2 = st.tabs(["Deep Research Results", "Raw Data"])
            
            with tab1:
                st.header("Deep Research Report")
                deep_research = st.session_state.results["deep_research"]
                
                # Display main query and topics
                st.subheader("Research Overview")
                st.markdown(f"**Main Query:** {deep_research.get('main_query', 'N/A')}")
                st.markdown(f"**Topics Researched:** {', '.join(deep_research.get('topics', []))}")
                
                # Display synthesis
                synthesis = deep_research.get("synthesis", {})
                
                st.subheader("Cross-Topic Synthesis")
                st.markdown(synthesis.get("synthesis_summary", "No synthesis summary available."))
                
                # Display cross-topic insights
                st.subheader("Key Insights Across Topics")
                for insight in synthesis.get("cross_topic_insights", []):
                    st.markdown(f"- {insight}")
                
                # Display implications
                st.subheader("Implications for Main Query")
                st.markdown(synthesis.get("implications_for_main_query", "No implications available."))
                ...
                for rec in synthesis.get("recommendations", []):
                    if isinstance(rec, dict):
                        ...
                        if rec.get("supporting_topics"):
                            st.markdown(f"  *Supporting Topics:* {', '.join(rec['supporting_topics'])}")
                    else:
                        st.markdown(f"- {rec}")
                
                # Display individual topic results in expandable sections
                st.subheader("Topic Details")
                topic_results = deep_research.get("topic_results", {})
                for topic, data in topic_results.items():
                    with st.expander(f"Topic: {topic}"):
                        ...
                        st.markdown("**Specialized Insights:**")
                        for insight in insights.get("specialized_insights", []):
                            st.markdown(f"- **{insight.get('insight', 'N/A')}**")
                            st.markdown(f"  *Explanation:* {insight.get('explanation', 'N/A')}")
                            ...
                            if insight.get("connections"):
                                st.markdown("  *Connections to Main Query:*")
                                for conn in insight["connections"]:
                                    st.markdown(f"  - {conn}")
                        
                        st.markdown("**Technical Details:**")
                        st.markdown(f"{insights.get('technical_details', 'Not available')}")
                        
                        st.markdown("**Integration with Main Query:**")
                        st.markdown(f"{insights.get('integration', 'Not available')}")
                        ...
                        ranked_sources = ranking.get("ranked_sources", [])
                        
                        if ranked_sources:
                            for i, source in enumerate(ranked_sources[:3]):
                                st.markdown(f"{i+1}. **{source.get('title', 'Untitled')}**")
                                st.markdown(f"   Score: {source.get('overall_score', 0.0):.2f}")
                                st.markdown(f"   [View Source]({source.get('url', '#')})")
                        else:
                            st.markdown("No ranked sources available for this topic.")
            
            with tab2:
                st.header("Raw Data")
                st.json(st.session_state.results)
        else:
            # Create tabs for standard research view
