...
def get_schedule():
    ...
    cursor.execute("""
        SELECT Irrigation, Fertiliser, IC, FC, Week 
        FROM Analysis 
        WHERE Username = %s
        ORDER BY Week DESC 
        LIMIT 1
    ...
    if result:
        return jsonify({
            'irrigation': result[0],
            'fertilizer': result[1],
            'irrigationComplete': bool(result[2]),
            'fertilizerComplete': bool(result[3]),
            'week': result[4]
        })
